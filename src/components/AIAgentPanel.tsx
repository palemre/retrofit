...
export default function AIAgentPanel({ project }: AIAgentPanelProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {isOpen && (
        <section
          id="retrofit-insight-agent"
          className="bg-white rounded-2xl shadow-2xl border border-green-100 p-6 w-80 sm:w-96 max-w-full"
        >
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <button
                ...
                aria-label="Close RetroFit Insight Agent"
              >
                ×
              </button>
            </div>
      ...
      <button
        ...
        className={`rounded-full shadow-lg font-semibold transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 ${
          isOpen
            ? 'bg-green-100 text-green-800 px-4 py-2 text-sm'
            : 'bg-green-600 hover:bg-green-700 text-white px-6 py-3 text-base'
        }`}
        aria-expanded={isOpen}
        aria-controls={isOpen ? 'retrofit-insight-agent' : undefined}
      >
        {isOpen ? 'Hide agent' : 'Chat with RetroFit Insight Agent'}
      </button>
    </div>
  );
}
